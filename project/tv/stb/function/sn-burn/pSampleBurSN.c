...
static D_Result sn_burn_handle(D_HSerial sl_hdl) D_FUNCPOSTFIT
{
	...
	if(/*d_serial_read(sl_hdl, serial_data->buf_rciv, serial_data->buf_len)*/rcv_len > 0)	//从串口中接收到数据
	{
		...
		if(((valid_len + HEADER_TAIL_LEN) > serial_data->buf_len)||(valid_len > D_CONF_DEF_STB_ID_LEN))
		{
		...
		memcpy(serial_data->loaderparam.stb_param_copy.stb_id, &(serial_data->buf_rciv[8]),valid_len);
