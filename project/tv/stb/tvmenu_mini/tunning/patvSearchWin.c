...
static char* D_DisplayMode[] = {"PAL", "NTSC", "SECAM"};
static char* SoundMode_String[3][5] =
{
    {"BG", "DK", "I","M","N"},
    {"M","BG", "DK", "I"},
    {"BG", "DK", "I", "M"},
};
static char SoundMode[3][5] =
{
    {D_AudioBG, D_AudioDK, D_AudioI,D_AudioM,D_AudioN},
    {D_AudioM,D_AudioBG,D_AudioDK,D_AudioI},
    {D_AudioBG, D_AudioDK, D_AudioI, D_AudioM},
};

static D_U16 SoundModeCnt[3] = {5, 4, 4};
#else
...
/*开始节目搜索*/
static D_Result
start_atv_manual_scan(D_U32 ifFreq, D_VideoMode  videoMode, D_AudioMode audioMode, D_U8 flag, D_Bool up_flag)D_FUNCPOSTFIT
{
    D_Frame *frm = &atv_search_window;
    D_Visual *win;
            ...
            switch(evt->fp.key)
            {
                    ...
                    //if(search_info->hscan == D_INVALID_HANDLE)
                {
                    ...
                    else
                    {
                        if(search_info->hscan == D_INVALID_HANDLE)
                        {
                            start_atv_manual_scan(search_info->fparam.freq, search_info->video_mode, search_info->audio_mode, search_info->skip, D_FALSE);
                            gStop_search_flag = 1;
                        }
                ...
                case D_KEY_PNL_R:
                {
                    ...
                    else
                    {
                        if(search_info->hscan == D_INVALID_HANDLE)
                        {
                            start_atv_manual_scan(search_info->fparam.freq, search_info->video_mode, search_info->audio_mode, search_info->skip, D_TRUE);
                            gStop_search_flag = 0;
                            // sprintf(search_info->manual_strnum, "%0.2f", search_info->tune_val);
                            // d_label_set_text(search_info->manual_text_vis, search_info->manual_strnum);
                        }
        ...
        case D_EVT_FP:
        {
            switch(evt->fp.key)
            {
                ...
                case D_KEY_PNL_L:
                {
                    ...
                    if(search_info->tune_val < 42750)
                    {
                        search_info->tune_val = 869200;
                    }
                ...
                {
                    search_info->tune_val += 50;
                    if(search_info->tune_val > 869250)
                    {
                        search_info->tune_val = 42800;
                    }
                    sprintf(search_info->tune_strnum, "%d.%02d", (search_info->tune_val / 1000), ((search_info->tune_val % 1000)/10));
                    d_label_set_text(search_info->tune_text_vis, search_info->tune_strnum);
                    d_label_set_text(search_info->manual_text_vis, search_info->tune_strnum);
