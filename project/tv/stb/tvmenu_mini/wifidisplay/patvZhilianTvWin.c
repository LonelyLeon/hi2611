...
#define WIDTH_ALL 355
#define HEIGHT_HINTINFO1 55
#define HEIGHT_HINTINFO2 110
#define COORD_X 595
#define COORD_Y_HINTINFO1 75
#define COORD_Y_HINTINFO2 128
...
static D_ZhiLianInstructionInfo ZhiLianInstructionInfo_eng[] =
{
    ...
    },
    {
        ...
        "Step2: Click on the drop-down button on the mobile player, select \"% s\" in the device drop-down table, and click on the connection.",
        "",
    },
    {
        "Instructions",
...
atv_zhilian_func_icon_draw(D_Visual *vis, D_U32 type) D_FUNCPOSTFIT
{
    ...
    {
        d_rect_set(&sr, 0, 0, img->buf.width, img->buf.height);
        d_rect_set(&dr, 111, 93, img->buf.width, img->buf.height);
        d_vis_draw_image (vis, &dr, img, &sr);
    }
    ...
    {
        d_rect_set(&sr, 0, 0, img->buf.width, img->buf.height);
        d_rect_set(&dr, 174, 0, img->buf.width, img->buf.height);
        d_vis_draw_image (vis, &dr, img, &sr);
    }
#if 0
...
atv_zhilian_show_function(D_U32 type, D_U32 action) D_FUNCPOSTFIT
{
    ...
    d_label_set_text( vis, ZhiLianInstructionInfo[type].strInstruction); //ZhiLianInstructionInfo[type].strInstruction);

    vis = d_frame_get_control( &atv_zhilian_tv_window, ID_ZHILIANTV_LABEL_HINTINFO_1);
    if(type == 1)
        d_vis_set_pos(vis, COORD_X, COORD_Y_HINTINFO1, WIDTH_ALL, HEIGHT_HINTINFO1);
    ...
    if(type == 1)
        d_vis_set_pos(vis, COORD_X, COORD_Y_HINTINFO2, WIDTH_ALL, HEIGHT_HINTINFO2);
    ...
    //(type != 1) ? (d_vis_show(vis, D_TRUE)) : (d_vis_show(vis, D_FALSE));

    vis = d_frame_get_control( &atv_zhilian_tv_window, ID_ZHILIANTV_IMAGE_SCREEN_ICON);
    (type == 1) ? (d_vis_show(vis, D_TRUE)) : (d_vis_show(vis, D_FALSE));
