...
medialist_menu_get_files_list(D_Char* curr_path) D_FUNCPOSTFIT
{
    if(curr_path[0])
    {
        ...
        if(uCurrentMenuTpe == ATV_MEDIA_MENU_STATUS_MOVIE)
        {
            type = media_movie_type;
            cnt = sizeof(media_movie_type) / sizeof(media_movie_type[0]);
        }
...
medialist_menu_ok_action (void) D_FUNCPOSTFIT
{
    ...
    else if(Judge_Movie_File(&(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus]))\
    || Judge_TS_File(&(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus])))
    {
        /* 播放媒体文件*/
        if(D_OK == p_media_movie_full_screen_show(media_list_data->mediaCurrFolder_utf8, media_list_data->mediaCurrFocus, &media_list_data->mediaFileRset, 0, media_movie_fullscreen_hide_handler))
        {
