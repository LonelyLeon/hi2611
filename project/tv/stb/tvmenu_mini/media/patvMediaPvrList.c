...
#include "pvr_list_menu_window.c"
...
typedef struct
{
    ...
    D_Char  mediaFolderShow[MEDIA_LIST_NUM_PER_PAGE][MEDIA_FILE_NAME_LEN];/**文件详细信息**/
...
} D_MediaPvrListMenuData;

static D_MediaPvrListMenuData* media_list_data = NULL;
...
static PREFIXCONST D_CtrlAttribute POSTCONST GUID_Attr_pvr_normal = { -1, atv_IMG_ID_media_video, -1, NULL, };
static PREFIXCONST D_CtrlAttribute POSTCONST GUID_Attr_pvr_focus = { -1, atv_IMG_ID_media_video_focse, -1, NULL, };
...
static PREFIXCONST D_FrameCtrlDescr POSTCONST GUID_Descr_pvr = {{&GUID_Attr_pvr_normal, &GUID_Attr_pvr_focus, NULL, }, 1, };
...
medialist_menu_get_files_list(D_Char* curr_path) D_FUNCPOSTFIT
{
    if(curr_path[0])
    {
        ...
        if(uCurrentMenuTpe == ATV_MEDIA_MENU_STATUS_PVR)
        {
            type = media_pvr_type;
            cnt = sizeof(media_pvr_type) / sizeof(media_pvr_type[0]);
        }
        //   p_file_scan_get_current_dir(curr_path, &media_list_data->mediaFileRset, type, cnt, D_FILE_MATCH_MODE_FILETYPE);
        p_file_scan_get_files_by_type(curr_path, &media_list_data->mediaFileRset, type, cnt, D_FILE_MATCH_MODE_FILETYPE);
    }
    else
    {
...
medialist_menu_show_file_name(void) D_FUNCPOSTFIT
{
    ...
    {
        sprintf(media_list_data->mediaCurrPathName_utf8, "%s", media_list_data->mediaFolderShow[media_list_data->mediaCurrFocus]);
...
static D_Result
media_analyse_ts_name(D_ID row, char* file_name) D_FUNCPOSTFIT
{
    memset(media_list_data->mediaListContent[row], 0x00, sizeof(media_list_data->mediaListContent[row]));
    memcpy(media_list_data->mediaListContent[row], file_name, strlen(file_name));

    if((media_list_data->mediaListContent[row] != NULL) && (strlen(media_list_data->mediaListContent[row]) == 2))
    {
        memset(media_list_data->mediaFileName, 0x00, sizeof(media_list_data->mediaFileName));
        memcpy(media_list_data->mediaFileName, media_list_data->mediaListContent[row], sizeof(media_list_data->mediaListContent[row]));
        //media_list_data->mediaCurrPathName_utf8 = NULL;
        sprintf(media_list_data->mediaFolderShow[row], "%s",  media_list_data->mediaListContent[row]);
        return D_OK;
    }

    if(media_list_data->mediaListContent[row] != NULL)
    {
        char* temp = NULL;
        char* ch_pos = NULL;
        ...
        ch_pos = strrchr(media_list_data->mediaListContent[row], '/');
        if(ch_pos)
            ch_pos++;
        else
            return D_ERR;
        ...
        memset(media_list_data->mediaFileName, 0x00, sizeof(media_list_data->mediaFileName));
        memcpy(media_list_data->mediaFileName, ch_pos, strlen(ch_pos));
        ...
        if(ch_pos)
        {
            ...
            if(temp && (strlen(temp) == 15))
            {
                *temp = '\0';
                ...
                /* 记录utf8的文件夹名*/
                sprintf(media_list_data->mediaFolderShow[row], "%s  %s.%s.%s %s:%s:%s",
                ...
                {
                    D_Size file_length;
                    file_length = strlen(media_list_data->mediaFileRset.ids[row].path.filename);
                    memset(media_list_data->mediaListContent[row], 0x00, sizeof(media_list_data->mediaListContent[row]));
                    p_cc_utf8_to_sys((D_U8*)media_list_data->mediaListContent[row], MEDIA_FILE_NAME_LEN, \
                                     media_list_data->mediaFileName, sizeof(media_list_data->mediaFileName)); // (D_U8*)fattr->path.filename, file_length);
                }
#else
                strcpy(media_list_data->mediaListContent[row], media_list_data->mediaFileName); //fattr->path.filename);
#endif
            }
        }
        return D_OK;
        ...
        else
        {
            ...
            switch(fattr->class_type)
            {
                ...
                case D_FILE_SCAN_FMT_UpDir:
                    ...
                    break;
                case D_FILE_SCAN_FMT_Pvr:
                    // case D_FILE_SCAN_FMT_TS:
                    d_vis_set_attribute(vis_image, (D_Ptr)&GUID_Descr_pvr);
                    break;
                case D_FILE_SCAN_FMT_Device:
                    d_vis_set_attribute(vis_image, (D_Ptr)&GUID_Descr_disk);
                    break;
            }
            media_analyse_ts_name(id, media_list_data->mediaFileRset.ids[id].path.filename);
...
medialist_menu_enter_next_folder( void )D_FUNCPOSTFIT
{
    ...
    if(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus].type == 0  &&  media_list_data->mediaCurrFocus < media_list_data->mediaFileRset.cnt)
    {
        ...
        curr_device[0] = media_list_data->mediaCurrPartition + 'C';
        sprintf(media_list_data->mediaCurrFolder_utf8, "%s", curr_device);
...
/** @brief 退出全屏播放时回调*/
static D_Result
pvr_movie_fullscreen_hide_handler(D_U16 focus)D_FUNCPOSTFIT
{
    if(media_list_data)
    {
        d_media_pvr_stop();
...
medialist_menu_ok_action (void) D_FUNCPOSTFIT
{
    ...
    else if(Judge_PVR_File(&(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus])))
    {
        /* 播放媒体文件*/
        if(D_OK == p_media_pvr_full_screen_show(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus].path.filename,  &media_list_data->mediaFileRset, media_list_data->mediaCurrFocus, pvr_movie_fullscreen_hide_handler))
        {
...
/** @brief 窗口初始化*/
D_Result
pvr_list_menu_window_init(D_Frame *frm) D_FUNCPOSTFIT
{
    D_Visual *vis;

    media_list_data = (D_MediaPvrListMenuData*)PROJECT_ALLOC(sizeof(D_MediaPvrListMenuData));
    if(!media_list_data)
    {
        return D_ERR;
    }

    memset(media_list_data, 0x00, sizeof(D_MediaPvrListMenuData));
    media_list_data->mediaCurrPartition = 0xFF;
    media_list_data->mediaCurrFocus = 0;
...
{
    if(d_frame_is_loaded(&pvr_list_menu_window))
    {
        return D_OK;
    }
    uCurrentMenuTpe = ATV_MEDIA_MENU_STATUS_PVR;
    D_DUMP("p_atv_media_movie_show \n");
    return d_gui_enter_menu_new(ATV_MEDIA_MENU_ID, &pvr_list_menu_window, D_CLEAR_NONE);
}
