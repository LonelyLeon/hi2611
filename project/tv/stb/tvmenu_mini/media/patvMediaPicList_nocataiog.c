...
medialist_menu_enter_next_folder( void )D_FUNCPOSTFIT
{
    ...
    media_list_data->area_num = p_tv_file_scan_get_typefile_by_volumes(media_list_data->cur_rset_num, &(media_list_data->fileRsetInfo), media_list_data->area_info, D_FILE_SCAN_FMT_Photo);
...
medialist_menu_ok_action (void) D_FUNCPOSTFIT
{
    ...
    else if((media_list_data->fileRsetInfo[media_list_data->area_info[media_list_data->mediaCurrFocus]].class_type == D_FILE_SCAN_FMT_Photo))
    {
        /* 播放媒体文件*/
        if(D_OK == p_pic_full_screen_menu_show(media_list_data->mediaCurrPathName_utf8, media_list_data->area_info,media_list_data->area_num,media_list_data->fileRsetInfo, media_list_data->mediaCurrFocus, media_pic_fullscreen_hide_handler))
        {
