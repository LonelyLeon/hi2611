    ...
    switch (evt->type)
    {
        case D_EVT_FP:
        {
            switch (evt->fp.key)
            {
                case D_KEY_DOWN:
                case D_KEY_PNL_D:
		  		case D_KEY_NEXT:		//add NextKey 20190705 Yin
                {
                ...
                case D_KEY_UP:
                case D_KEY_PNL_U:
		  		case D_KEY_LAST:			//add NextKey 20190705 Yin
                {
                    d_image_box_set_image(picture_full_data->vis_loading, NULL);
					p_show_osd_window(D_FALSE);
                    if(get_full_pic_play_state() == 1)
                ...
                case D_KEY_BACK:
                case D_KEY_ESC:
		  		case D_KEY_STOP:			//add NextKey 20190705 Yin
                {
                    if (fullscreen_prompt_msg_flag)
                    {
                        d_atv_pop_msg_hide(D_POP_MSG_PRIO_MSG_2);
...
/** @brief 全屏播放图片控制窗口显示*/
D_Result p_pic_full_screen_menu_show(D_Char *path , D_FileScan *fs, D_ID index, D_Result (*handler) (D_U16 param)D_FUNCPOSTFIT)D_FUNCPOSTFIT
{
    D_Char *tok;
    D_Result ret = D_ERR;
