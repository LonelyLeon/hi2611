...
media_find_next_by_play_mode(MEDIA_MUSIC_PLAY_MODE play_mode)D_FUNCPOSTFIT
{
    ...
    switch(play_mode)
    {
        ...
        case MEDIA_MUSIC_LIST_LOOP:
            ...
            for(idx = media_list_data->mediaMusicCurrPlaying + 1; idx < media_list_data->mediaFileRset.cnt; idx++)
            {
                if(Judge_Music_File(&(media_list_data->mediaFileRset.ids[idx])))
                {
                    play_id = idx;
                    break;
                }
        ...
        case MEDIA_MUSIC_RANDOM_PLAY:
            ...
            while(1)
            {
                idx = rand() % (media_list_data->mediaFileRset.cnt);
                if(Judge_Music_File(&(media_list_data->mediaFileRset.ids[idx])))
                {
                    play_id = idx;
                    break;
...
medialist_menu_get_files_list(D_Char* curr_path) D_FUNCPOSTFIT
{
    if(curr_path[0])
    {
        ...
        if(uCurrentMenuTpe == ATV_MEDIA_MENU_STATUS_MUSIC)
        {
            type = media_music_type;
            cnt = sizeof(media_music_type) / sizeof(media_music_type[0]);
        }
...
#ifndef D_SYSTEM_WIN32
        file_length = strlen(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus].path.filename);
        p_cc_utf8_to_sys((D_U8*)media_list_data->mediaFileName, MEDIA_FILE_NAME_LEN, \
        (D_U8*)media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus].path.filename, file_length);
#else
        strcpy(media_list_data->mediaFileName, media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus].path.filename);
#endif
...
media_play_curr_music(D_U16 id) D_FUNCPOSTFIT
{
    ...
    media_music_stop_play();
    fsattr = &(media_list_data->mediaFileRset.ids[id]);
...
medialist_menu_ok_action (void) D_FUNCPOSTFIT
{
    ...
    else if(Judge_Music_File(&(media_list_data->mediaFileRset.ids[media_list_data->mediaCurrFocus])))
    {
