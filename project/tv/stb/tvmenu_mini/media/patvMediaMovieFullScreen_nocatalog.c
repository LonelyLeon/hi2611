...
typedef struct
{
    ...
    D_U16 area_num;
    D_U16 *area_info;
    TV_FILE_RSET_INFO*  mediaFileScan;
    D_Char  mediaFileName_temp[MEDIA_FULL_MOVIE_PATH_NAME_LEN];    /**临时当前文件名**/
...
static D_Result
media_play_analyse_name(D_Char * file_name, D_Char * file_path)
{
    char * ch_pos = NULL;
...
static void media_full_movie_set_info(D_Bool TS, D_Bool valid)
{
    ...
    //name
    p_cc_utf8_to_sys((D_U8*)filename, D_PATH_LENGTH, (D_U8*)media_full_movie_data->mediaFullMoviePathName, strlen(media_full_movie_data->mediaFullMoviePathName));
    media_play_analyse_name(media_full_movie_data->mediaFileName_temp, filename);

    d_label_set_text(media_full_movie_data->vis_name, media_full_movie_data->mediaFileName_temp);
...
media_full_movie_analyse_file_path(void)
{
    ...
    if(length)
    {
        ...
        strcat(media_full_movie_data->mediaFullMoviePathName, media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[media_full_movie_data->mediaFullMovieCurrIndex]].path);
    }
    ...
    if(flag)
    {
        switch(play_mode)
        {
            ...
            case MEDIA_MOVIE_LIST_LOOP:
                ...
                for(idx = media_full_movie_data->mediaFullMovieCurrIndex + 1; idx < media_full_movie_data->area_num; idx++)
                {
                    if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx]].class_type == D_FILE_SCAN_FMT_Movie)
                    {
                        play_id = idx;
                        break;
                    }
                    else if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx]].class_type == D_FILE_SCAN_FMT_TS)
                    {
                        play_id = idx;
                        break;
                    }
            ...
            case MEDIA_MOVIE_RANDOM_PLAY:
                ...
                while(1)
                {
                    idx = rand() % (media_full_movie_data->area_num);
                    if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx]].class_type == D_FILE_SCAN_FMT_Movie)
                    {
                        play_id = idx;
                        break;
    ...
    else
    {
        switch(play_mode)
        {
            case MEDIA_MOVIE_SINGLE_LOOP:
                ...
                break;
            case MEDIA_MOVIE_LIST_LOOP:
                for(idx = media_full_movie_data->mediaFullMovieCurrIndex; idx > 0; idx--)
                {
                    if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx - 1]].class_type == D_FILE_SCAN_FMT_Movie)
                    {
                        play_id = idx - 1;
                        break;
                    }
                    else if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx - 1]].class_type == D_FILE_SCAN_FMT_TS)
                    {
                        play_id = idx-1;
                        break;
                    }
                ...
                if(play_id == -1)
                {
                    for(idx = media_full_movie_data->area_num - 1; idx >= media_full_movie_data->mediaFullMovieCurrIndex; idx--)
                    {
                        if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[idx]].class_type == D_FILE_SCAN_FMT_Movie)
                        {
                            play_id = idx;
...
media_full_movie_play(D_Bool first)D_FUNCPOSTFIT
{
    ...
    if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[media_full_movie_data->mediaFullMovieCurrIndex]].class_type == D_FILE_SCAN_FMT_Movie)
    {
        ...
        media_full_movie_data->mediaFullMovieWin.w, media_full_movie_data->mediaFullMovieWin.h))
        {
            ...
            if(D_OK == d_media_movie_play(media_full_movie_data->mediaFullMoviePathName, media_full_movie_data->mediaFullMovieWin.x, media_full_movie_data->mediaFullMovieWin.y, \
                                          media_full_movie_data->mediaFullMovieWin.w, media_full_movie_data->mediaFullMovieWin.h, 0, NULL))
            {
                play_play_test_times = d_sys_get_time();
                media_full_movie_set_info(D_FALSE, D_TRUE);
                if(first == D_FALSE)
                {
                    media_full_movie_menu_show_en(MEDIA_FULL_MOVIE_MENU_SHOW);
    ...
    else if(media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[media_full_movie_data->mediaFullMovieCurrIndex]].class_type == D_FILE_SCAN_FMT_TS)
    {       
        D_Result rest;
        d_mediacodec_close();
...
media_full_movie_play_by_mode(D_U8 flag)D_FUNCPOSTFIT
{
    ...
    if(file_id >= 0)
    {
        ...
        strcpy(media_full_movie_data->mediaFullMoviePathName, media_full_movie_data->mediaFileScan[media_full_movie_data->area_info[media_full_movie_data->mediaFullMovieCurrIndex]].path);
...
D_Result
p_media_movie_full_screen_show_1 (D_Char* path_name, D_U16 movie_index, D_U16* area_info,  D_U16 area_num, TV_FILE_RSET_INFO* fileScan, D_U8 play_mode, D_Result (*mediaFullMovieHideHandler) (D_U16 param)) D_FUNCPOSTFIT
{
    ...
    if((path_name != NULL) && (fileScan != NULL) && (mediaFullMovieHideHandler != NULL))
    {
        ...
        media_full_movie_data->area_info = area_info;
        media_full_movie_data->area_num = area_num;
