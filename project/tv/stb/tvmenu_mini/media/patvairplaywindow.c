...
/**airplay模式*/
#define AIRPLAY_MODE_MIRROR (0)
#define AIRPLAY_MODE_MUSIC (1)
#define AIRPLAY_MODE_MOVIE (2)

typedef struct
{
    D_HTimer dlnaFullMovieDestroyTimer;  /**关闭菜单定时器*/
    ...
    D_Visual* vis_prompt;
    D_Visual* vis_prompt_mirror;
    D_U32 miror_prompt_cnt;
    D_Bool airplay_mode; // 0: mirror, 1: audio, 2: movie
} DlnaWindowInfo;
...
static D_Char*strPlayMirrorWait[6] =
{
    "Airplay...",
    "Airplay.  ",
    "Airplay.. ",
...
static void (*menu_cb)(D_U32 action) = NULL;
...
/** @brief 复位显示定时器*/
static D_Result
dlna_full_movie_reset_destroy_timer(D_Bool enable) D_FUNCPOSTFIT
{
    //D_DUMP("dlna_full_movie_reset_destroy_timer %d\n", enable);
    if(dlna_window_info)
    {
        if(enable)
        {
            if(dlna_window_info->dlnaFullMovieDestroyTimer == D_INVALID_HANDLE)
            {
                dlna_window_info->dlnaFullMovieDestroyTimer = d_timer_create(NULL, 800, d_gui_get_queue());
                if(dlna_window_info->dlnaFullMovieDestroyTimer != D_INVALID_HANDLE)
                {
                    d_vis_subscribe_evt(dlna_window_info->vis_full_win, D_EVT_TIMER_MASK, dlna_window_info->dlnaFullMovieDestroyTimer);
                }
            }
            if(dlna_window_info->dlnaFullMovieDestroyTimer != D_INVALID_HANDLE)
            {
                d_timer_reset(dlna_window_info->dlnaFullMovieDestroyTimer, 800);
            }
        }
        else
        {
            if(dlna_window_info->dlnaFullMovieDestroyTimer != D_INVALID_HANDLE)
            {
                d_vis_unsubscribe_one_evt(dlna_window_info->vis_full_win, D_EVT_TIMER_MASK, dlna_window_info->dlnaFullMovieDestroyTimer);
                d_timer_destroy(dlna_window_info->dlnaFullMovieDestroyTimer);
                dlna_window_info->dlnaFullMovieDestroyTimer = D_INVALID_HANDLE;
            }
        }
    }
    return D_OK;
...
/** @brief 复位显示定时器*/
static D_Result
dlna_full_movie_reset_play_timer(D_Bool enable) D_FUNCPOSTFIT
{
    if(dlna_window_info)
    {
        if(enable)
        {
            ...
            if(dlna_window_info->dlnaFullMoviePlayTimer != D_INVALID_HANDLE)
            {
                d_timer_reset(dlna_window_info->dlnaFullMoviePlayTimer, DLNA_FULL_MOVIE_PLAY_REFRESH_TIME);
            }
        }
        else
        {
            if(dlna_window_info->dlnaFullMoviePlayTimer != D_INVALID_HANDLE)
            {
                d_vis_unsubscribe_one_evt(dlna_window_info->vis_full_win, D_EVT_TIMER_MASK, dlna_window_info->dlnaFullMoviePlayTimer);
...
/** @brief 播放视频*/
static D_Result
dlna_full_movie_play_mirror_music(D_Bool first, D_U32 music)D_FUNCPOSTFIT
{
    dlna_window_info->airplay_mode = (music) ? AIRPLAY_MODE_MUSIC : AIRPLAY_MODE_MIRROR;    
    ...
    d_label_set_text(dlna_window_info->vis_prompt, NULL);
    ...
    dlna_full_movie_reset_play_timer(D_FALSE);    
    dlna_window_info->miror_prompt_cnt = 0;    
    if(first || dlna_window_info->airplay_mode ==AIRPLAY_MODE_MUSIC)
    {
        d_label_set_text(dlna_window_info->vis_prompt_mirror, strPlayMirrorWait[0]);
        d_vis_show(dlna_window_info->vis_prompt_mirror, D_TRUE);
        dlna_full_movie_reset_play_timer(D_TRUE);  
    }
    return D_OK;
}
...
/** @brief 播放视频*/
static D_Result
dlna_full_movie_play_movie(D_Bool first)D_FUNCPOSTFIT
{
    D_Result ret = D_ERR;
    D_U32 t = d_sys_get_time();

    dlna_window_info->airplay_mode = AIRPLAY_MODE_MOVIE;
    ...
    dlna_full_movie_menu_show_en(DLNA_FULL_MOVIE_MENU_HIDE);
    d_vis_show(dlna_window_info->vis_prompt, D_FALSE);
...
_MOVIE_PLAY_END:
    ...
    d_label_set_text(dlna_window_info->vis_prompt_mirror, NULL);
    d_vis_show(dlna_window_info->vis_prompt_mirror, D_FALSE);
...
dlna_full_movie_set_play_progress_value(D_U32  key_value, D_Bool repeat) D_FUNCPOSTFIT
{
    ...
    vis = d_frame_get_control (&atv_airplay_window, ID_MOVIE_SELECT_POS);
    if(vis)
    {
...
dlna_full_movie_set_progress(void)
{
    ...
    if(dlna_window_info->dlnaFullMovieInfo.totaltime && dlna_window_info->airplay_mode == AIRPLAY_MODE_MOVIE)
    {
        total_time_value = dlna_window_info->dlnaFullMovieInfo.totaltime;
        cur_set_time = (dlna_window_info->progressCur_X - dlna_window_info->progressStart_X) * total_time_value / (dlna_window_info->progressEnd_X - dlna_window_info->progressStart_X);
...
/** @brief 窗口事件处理*/
D_Result
atv_airplay_window_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    D_U32 status;
    D_Visual *pvis;
    ...
    switch(evt->type)
    {
        case D_EVT_FP:
        {
            ...
            switch(evt->fp.key)
            {
                ...
                case D_KEY_PNL_L:
                case D_KEY_LEFT:
                    if(dlna_window_info->airplay_mode != AIRPLAY_MODE_MOVIE)
                    {
                        return D_OK;
                    }
        ...
        break;
        case D_EVT_TIMER:
            if(evt->timer.htimer == dlna_window_info->dlnaFullMovieDestroyTimer)
            {
                D_DUMP("dlnaFullMovieDestroyTimer\n");
                p_atv_airplay_window_hide();
                break;
            }
            else if(evt->timer.htimer == dlna_window_info->dlnaFullMovieMenuTimer)
            {
                if(dlna_window_info->dlnaFullMovieMenuEnable == DLNA_FULL_MOVIE_MENU_SHOW && dlna_window_info->menu_auto_close == D_FALSE)
                {
                    D_U32 cur_t;
                ...
                /*播放失败时，5秒时通知协议，15秒后关闭菜单*/
                if(dlna_window_info->menu_auto_close)
                {
                    if(dlna_window_info->timer_counter == 1)
                    {
                        notify_protocol_status(PLAYER_STATUS_FINISHED);
                    }
                    else if(dlna_window_info->timer_counter >= 3)
                    {
                        dlna_window_info->menu_auto_close = D_FALSE;
                        p_atv_airplay_window_hide();
                        break;                        
            ...
            else if(evt->timer.htimer == dlna_window_info->dlnaFullMoviePlayTimer && dlna_window_info->menu_auto_close == D_FALSE)
            {
                ...
                {
                    D_U32 textidx;
                    dlna_window_info->miror_prompt_cnt++;
                    textidx = (dlna_window_info->miror_prompt_cnt - 1) % 3;
                    textidx = (dlna_window_info->airplay_mode == AIRPLAY_MODE_MUSIC) ? (textidx + 3) : textidx;
                    d_label_set_text(dlna_window_info->vis_prompt_mirror, strPlayMirrorWait[textidx]);
                    d_vis_show(dlna_window_info->vis_prompt_mirror, D_TRUE);
                    break;
                }
                ...
                if(status == MOVIE_STATUS_PLAY)
                {
                    ...
                    if(dlna_window_info->dlnaFullMovieCurrTimeValue >= (dlna_window_info->dlnaFullMovieTotalTimeValue + 1))
                    {
                        ...
                        if(dlna_window_info->dlnaFullMovieCheckEndCounter >= 5)
                        {
                            ...
                            d_media_movie_stop();
                            notify_protocol_status(PLAYER_STATUS_FINISHED);
                            dlna_full_movie_reset_destroy_timer(D_TRUE);
                            break;
                        }
                    }
            ...
            break;
        case D_EVT_USER1:
            if(evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_VOLUME)
            {
            ...
            else if((evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_MOVIE_SEEK) && dlna_window_info->menu_auto_close == D_FALSE && dlna_window_info->dlnaFullMovieTotalTimeValue > (D_U32)evt->user.param2)
            {
                ...
                pvis = d_frame_get_control (&atv_airplay_window, ID_MOVIE_SELECT_POS);
                if(pvis)
                {
                    dlna_full_movie_update_set_time();
            ...
            else if(evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_MOVIE_STOP)
            {
                if(dlna_window_info->dlnaFullMovieMenuEnable == DLNA_FULL_MOVIE_MENU_SHOW)
                {
                    dlna_window_info->dlnaFullHasSet = D_FALSE;
            ...
            else if(evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_MOVIE_PLAY)
            {
                D_Char*  file_title = (D_Char *)evt->user.param3;
                D_Char*  path_name =  (D_Char *)evt->user.param2;
                /*停止播放*/
                d_media_movie_stop();
                dlna_full_movie_reset_destroy_timer(D_FALSE);
                ...
                dlna_window_info->progressCur_X = dlna_window_info->progressStart_X;
                if(D_OK != dlna_full_movie_play_movie(D_TRUE))
                {
                    d_media_movie_stop();
                    notify_protocol_status(PLAYER_STATUS_STOP);
                    return D_ERR;
            ...
            else if((evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_MOVIE_STATUS) && dlna_window_info->menu_auto_close == D_FALSE)
            {
                status = evt->user.param2;
                ...
                else if(status == PLAYER_STATUS_PAUSED)
                {
                    dlna_full_movie_set_play_status(DLNA_FULL_MOVIE_PAUSE_STATUS);
                }
            }
            else if(evt->user.param1 == USER_MSG_PARAM1_AIRPLAY_STATUS)
            {
                ...
                {
                    //D_DUMP("D_AIRPLAY_STATUS_START\n");
                    dlna_full_movie_play_mirror_music(D_FALSE, evt->user.param3);
                    dlna_full_movie_reset_destroy_timer(D_FALSE);
                }
                else if(status == D_AIRPLAY_STATUS_VID_OK)
                {
                    d_vis_show(dlna_window_info->vis_prompt_mirror, D_FALSE);
                    dlna_full_movie_reset_play_timer(D_FALSE);  
                }
...
/** @brief 窗口初始化*/
D_Result
atv_airplay_window_init(D_Frame *frm) D_FUNCPOSTFIT
{
    D_Visual *vis;
    ...
    dlna_window_info->vis_prompt_mirror = d_frame_get_control(frm, ID_AIRPLAY_PROMPT_MSG);
    d_vis_show(dlna_window_info->vis_prompt, D_FALSE);
    d_vis_show(dlna_window_info->vis_prompt_mirror, D_FALSE);
    dlna_window_info->dlnaFullHasSet = D_FALSE;
    ...
    //d_label_set_text(d_frame_get_control (frm, ID_FULL_MOVIE_HW), "分辨率");
    //d_label_set_text(dlna_window_info->vis_name, dlna_window_info->dlnaFullMovieTitle);
    d_vis_show(dlna_window_info->vis_info_group, D_FALSE);
    ...
    if(vis)
    {
        ...
        d_vis_subscribe_evt (vis, D_EVT_ALL_KEY_MASK , D_INVALID_HANDLE);
        d_vis_set_handler (vis, atv_airplay_window_handler);
    }
    return D_OK;
}
...
p_atv_airplay_window_show(D_Char*  file_path, D_U32 mode, void (*cb)(D_U32 action)) D_FUNCPOSTFIT
{
    ...
    DLNA_MENU_LOCK();
    menu_cb = cb;
    if (D_FALSE == d_frame_is_loaded (&atv_airplay_window))
    {
        if(file_path)
            D_DUMP("file_path(%s)\n", file_path);
        ...
        if(dlna_window_info)
        {
            ...
            dlna_window_info->airplay_mode = mode;
            dlna_window_info->dlnaFullMoviePlayTimer = D_INVALID_HANDLE;
            dlna_window_info->dlnaFullMovieMenuTimer = D_INVALID_HANDLE;
            ...
            //sprintf(dlna_window_info->dlnaFullMoviePathName, "%s", "https://vd4.bdstatic.com/mda-jda6u9wh7zurzbdh/sc/mda-jda6u9wh7zurzbdh.mp4");
            ret = d_gui_enter_menu_new(ATV_MEDIA_MENU_ID, &atv_airplay_window, D_CLEAR_NONE);
            if(ret == D_OK)
            {
                if(dlna_window_info->airplay_mode == AIRPLAY_MODE_MOVIE)
                {
                    ret = dlna_full_movie_play_movie(D_TRUE);
                    if(ret != D_OK)
                    {
                        d_media_movie_stop();
                    }
                ...
                {
                    d_media_movie_close();
                    dlna_full_movie_play_mirror_music(D_TRUE, dlna_window_info->airplay_mode);
                }
            }
...
p_atv_airplay_window_hide(void) D_FUNCPOSTFIT
{
    ...
    if (D_TRUE == d_frame_is_loaded (&atv_airplay_window))
    {
        D_DUMP("p_atv_airplay_window_hide\n");
        d_media_movie_stop();
        d_av_set_vid_blank(0, D_TRUE); /*黑屏*/
#if 0 /* 菜单之外调用，避免频繁media与dtv的切换*/
        if(dlna_window_info->media_open_flag)
...
{
    if ((D_TRUE == d_frame_is_loaded (&atv_airplay_window)) && dlna_window_info)
    {
        return D_TRUE;
    }
    return D_FALSE;
