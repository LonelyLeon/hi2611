...
#include "dvbc_chan_info_test_window.c"
...
show_init_param_info(D_DBTSInfo ts_info, D_DBSrvInfo srv_info)
{
    ...
    sprintf(chan_info->Frequency, "%03d", ts_info.param.freq);
	...
	if(p_conf_get_scan_config())
	{
	    ...
	    ts_info.param.dvbc.mod = D_MIN(ts_info.param.dvbc.mod, D_FEND_QAM256);
            type = (D_U16)ts_info.param.dvbc.mod;
            for(idx = 0; idx < chan_info->mode_cnt; idx++)
            {
               
                if(type == fendMode[idx].val)
                {
                    break;
                }
            }
	    d_select_box_set_val(chan_info->vis_mod, idx);
	}
    ...
    d_input_box_set_text(chan_info->vis_vpid, chan_info->VideoPID, strlen(chan_info->VideoPID));
     ...
     type = (D_U16)srv_info.vid_codec;
            for(idx = 0; idx < chan_info->VideoType_cnt; idx++)
            {
               
                if(type == VideoType[idx].val)
                {
                    break;
                }
            }
    d_select_box_set_val(chan_info->vis_vtype, idx);
    ...
    {
        if(chan_info->format == FormatType[idx].val)
        {
            type = idx;
            break;
        }
...
try_to_play_handler(D_Bool sel,  D_U32 param)
{
    ...
    d_gui_close_frame(&dvbc_chan_info_test_window);
...
/** @brief 事件处理*/
static D_Result dvbc_chan_info_test_window_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
   D_Char	led[5];
    ...
    switch (evt->type)
    {
        case D_EVT_FP:
            ...
            switch (evt->fp.key)
            	{
                case D_KEY_BACK:
                    return d_gui_close_frame(&dvbc_chan_info_test_window);
                    break;
                case D_KEY_ESC:
                    return d_gui_close_frame(&dvbc_chan_info_test_window);
                    break;
...
/** @brief 窗口初始化*/
D_Result dvbc_chan_info_test_window_init (D_Frame *frm) D_FUNCPOSTFIT
{
    D_Visual        *vis;
    D_DBSrvID   srv;
    D_U32 idx;
    ...
    if (vis)
    {
        ...
        d_vis_set_handler (vis, dvbc_chan_info_test_window_handler);
        /*创建前端定期检查定时器*/
	...
	chan_info->vis_baud = d_frame_get_control (frm, ID_INPUT_BAUD);
...
{
    return d_gui_open_frame(&dvbc_chan_info_test_window, D_GUI_LAYER_MESSAGE);
}
