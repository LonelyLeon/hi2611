...
D_Char *content[2]={"1","2"};
D_Char *aud_type[12]={"MPEG2","AC3","AAC","DRA","DTS","PCM","ADPCM","INTERPHONE","MP3","WMA","AMR","VORBIS"};
...
multi_audio_show_curr_voice_info(void)D_FUNCPOSTFIT
{
	...
	{
		d_select_box_set_content (multi_audio->vis_audio, content, 2);
		d_select_box_set_val(multi_audio->vis_audio, multi_audio->multi_audio_val);			
	}
	else
	{
		d_select_box_set_content (multi_audio->vis_audio, content, 1); 
...
multi_audio_callback(void)D_FUNCPOSTFIT
{
	...
	vis = d_frame_get_control(frm, ID_MULTIAUDIO_SELECT_AUDIO);
	if (vis)
	{
	...
	if ((srv != -1) && (d_dbase_get_srv_info (srv, &srv_info) >= 0)) 
	{
		...
		if(srv_info.aud_acc != ((D_U8)multi_audio->multi_audio_val))
		{
			...
			d_tv_set_aud_acc (player->htv, (D_U8)multi_audio->multi_audio_val);
		}
	} 
