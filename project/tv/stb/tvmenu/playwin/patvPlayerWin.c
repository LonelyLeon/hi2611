...
D_Bool ts2net_func_running = D_FALSE; /* 是否已启动ts2net功能*/
D_Bool ts2net_check_enable = D_TRUE; /* 是否允许ts2net功能开启*/
...
static void unsupported_aud_cb(D_U8 temp) D_FUNCPOSTFIT
{
#if 0
    d_atv_pop_msg_show(D_POP_MSG_PRIO_MSG_2, D_GUI_LAYER_MESSAGE, D_POP_MSG_MSG, NULL, \
                       "不支持的音频", 2000, D_POP_MSG_RESPONSE_ANY, D_FALSE, NULL, 0);
#endif
}
...
D_Result
p_atv_player_win_set_timers (void);

#ifdef D_TV_LIULINK_SOURCE
D_Result p_atv_enter_liulink(void);
D_Result p_atv_exit_liulink(void);
#endif
...
atv_player_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    ...
    switch (evt->type)
    {
        case D_EVT_FP:
            ...
            {
#ifdef D_TV_LIULINK_SOURCE
                if(p_conf_get_vid_input() == D_VID_IUTPUT_LIULINK)
                {
                    p_atv_exit_liulink();
                }
                else
            ...
            switch (evt->fp.key)
            {
                ...
                case D_KEY_PROJECT_EXT:
                {
                    if(p_conf_get_vid_input() != D_VID_IUTPUT_LIULINK)
                    {
                        D_ERROR("Start LiuLink\n");
                        p_sys_set_input_mode(p_conf_get_vid_input(), D_VID_IUTPUT_LIULINK);

                        D_ERROR("p_conf_set_vid_input is D_VID_IUTPUT_LIULINK\n");
                        p_conf_set_vid_input(D_VID_IUTPUT_LIULINK);

                        D_ERROR("  liulinkclient_mainapp_open\n");
...
//                        p_atv_zhilian_main_menu_window_show();
		...
		case D_KEY_IMAGE:
			input = p_conf_get_vid_input();
			ntsc_flag = p_vip_get_ntscflag();
			p_tvconf_get_cur_imagemode(&p_mode, p_vip_get_dbinput(input));
			p_mode = (p_mode + 1)%4;
			p_vip_set_ImageMode(input , p_mode , ntsc_flag);
			p_tvconf_set_cur_imagemode(&p_mode, p_vip_get_dbinput(input));
			
			p_atv_mode_input_show(0 , p_mode);
			break;
		case D_KEY_SOUND:
			input = p_conf_get_vid_input();
			p_tvconf_get_cur_audiomode(&s_mode, p_vip_get_dbinput(input));
			s_mode = (s_mode + 1)%4;
			for(i = 0; i < digital_adjust_param ; i++)
			{
				p_tvconf_get_audio_peq_set_param(&param[i],p_vip_get_dbinput(input),s_mode,i);
			}
			p_tvconf_get_cur_t_ffft(&nfft, p_vip_get_dbinput(input),s_mode);
			d_av_Audio_PEQ(0, param, digital_adjust_param, nfft, D_Digital_Audio);
			p_tvconf_set_cur_audiomode(&s_mode, p_vip_get_dbinput(input));

			p_atv_mode_input_show(1 , s_mode);
			break;
                default:
                    if (STB_STATUS_PLAYING() && PLAYER_CAN_CHANNEL_CHANGE(player))
                    {
...
atv_player_window_init (void) D_FUNCPOSTFIT
{
    ...
    d_av_set_unsupported_aud_cb((DAVGUICALLBACK) unsupported_aud_cb);
