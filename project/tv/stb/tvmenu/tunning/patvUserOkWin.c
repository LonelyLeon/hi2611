...
user_ok_window_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    ...
    vis_button = d_frame_get_control(frm, ID_USEROK_BUTTON_OK);
    switch (evt->type)
    {
        case D_EVT_FP:
        {
...
user_ok_window_init(D_Frame *frm)
{
    ...
    vis = d_frame_get_control(frm, ID_USEROK_BUTTON_OK); //确定复位
    if(vis)
    {
    ...
    vis = d_frame_get_control(frm, ID_USEROK_GROUP_BG);
    if(vis)
    {
