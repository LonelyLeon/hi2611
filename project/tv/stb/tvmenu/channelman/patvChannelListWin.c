...
static D_Result set_select_list_title(void)D_FUNCPOSTFIT
{
    ...
    if (vis)
    {
        ...
        {
            if(list_data->menu_lang == D_LANG_ENG)
            {
                d_label_set_text(vis, "Radio List"); 
            }
            else
            {
...
program_list_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    ...
    switch (evt->type)
    {
        case D_EVT_DATA_CHANGED:
            return chanlist_update_list(vis);
            break;
        case D_EVT_SELECTED:
            return D_OK;
            break;
...
channel_list_window_init (D_Frame *frm) D_FUNCPOSTFIT
{
    ...
    list_data->menu_lang = p_conf_get_lang();
    curr_play_srv = -1;
