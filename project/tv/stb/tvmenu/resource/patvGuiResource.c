...
#define _IMAGE_ID_MEDIA_START atv_IMG_ID_background
...
static D_Image *get_resource_image(int id, D_U32 *res_type, D_U32 *pRes_addr)
{
    ...
    if((id >= _IMAGE_ID_START) && (id != _IMAGE_ID_SINGLE_MAX) && (id != _IMAGE_ID_COMB_MAX))
    {
        if(id < _IMAGE_ID_SINGLE_MAX)
        {
            ...
            {
                D_DUMP("#####tv GUI load_res load image:%d,  %x, %d\n", id, sys_images[idx].image[img_lang].buffer, sys_images[idx].image[img_lang].size);
...
D_Result p_gui_atv_resource_init(void)
{
    ...
    for (i = _IMAGE_ID_START; i < endidx; i++)
    {
        ...
        {
            if(sys_images[idx].image[_LAN_CHN].buffer && sys_images[idx].image[_LAN_CHN].size && sys_images[idx].image[_LAN_CHN].img == NULL)
            {
