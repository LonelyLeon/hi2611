...
#include "atv_select_box_window.c"
...
static D_Char head_prompt[24];
static D_Coord def_posx = 0, def_posy = 0;
...
static D_Bool get_curr_sel(D_Visual *vis)
{
	...
	if(fvis)
	{
		focus = d_frame_get_control_id(&atv_select_box_window, fvis);
...
static D_Result set_default_sel(D_Bool def_sel)
{
	...
	vis = d_frame_get_control(&atv_select_box_window, focus);
	if(vis)
	{
		d_vis_set_focus(vis);
	}
...
static D_Result change_focus(D_Visual *vis)
{
	...
	if(fvis)
	{
		...
		if(focus == ID_BUTTON_0 || focus == ID_BUTTON_1)
		{
			...
			fvis = d_frame_get_control(&atv_select_box_window, focus);
			if(fvis)
			{
...
static D_Result prompt_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
	...
	{
		case D_EVT_TIMER:
                     if(curr_prio_selbox->prompt_htimer == evt->timer.htimer)
			{
				D_Int sec;
                
				curr_prio_selbox->timeout -= 1000;
                            curr_prio_selbox->timeout = D_MAX(0, curr_prio_selbox->timeout);
				sec = curr_prio_selbox->timeout / 1000;
				if(sec > 0)
				{
					snprintf(head_prompt, sizeof(head_prompt), "%d", sec);
				}
				else
				{
					snprintf(head_prompt, sizeof(head_prompt), "%s", " ");
				}
...
/*选择窗口初始化*/
D_Result atv_select_box_window_init(D_Frame *init) D_FUNCPOSTFIT
{
	D_Visual *vis;
	...
	{
		/*设置定时器*/
		if(curr_prio_selbox->timeout > 0 && curr_prio_selbox->timeout != ((D_MSec)-1))
		{
	...
	if(vis)
	{
		d_label_set_text(vis, curr_prio_selbox->msg);
	}

	vis = d_frame_get_control(init, ID_COUNTDOWN_LABEL);
	if(vis)
	{
		if(curr_prio_selbox->prompt == D_TRUE)
		{
			...
			d_vis_subscribe_evt(vis, D_EVT_TIMER_MASK, curr_prio_selbox->prompt_htimer);
                     snprintf(head_prompt, sizeof(head_prompt), "%d", curr_prio_selbox->timeout / 1000);
                     d_label_set_text(vis, head_prompt);
		}
              else
		{
...
{
	D_FrameControlDescr *descr = (D_FrameControlDescr*)(atv_select_box_window.ctrl_descrs);
    
	curr_prio_selbox = &selbox_data[prio];
	descr->x = curr_prio_selbox->posx;
	descr->paly = curr_prio_selbox->posy;     
	return D_OK;
}
		...
		D_U32 param, D_S32 posx, D_S32 posy) D_FUNCPOSTFIT
{
	...
	ret = d_pop_new(prio, layer, &atv_select_box_window, SelBoxShow);
	if(ret != D_OK)
	{
#ifdef D_DEBUG
	...
	pop->posx = posx;
	pop->posy = posy;
       if(posx < 0 || posy < 0 ||posx >= D_SCREEN_HOR_RESOLUTION || posy >= D_SCREEN_VER_RESOLUTION)
	{
		pop->posx = def_posx;
		pop->posy = def_posy;
	}
...
{
	return d_atv_pop_select_show_new (prio, layer, str_title, msg, timeout, def, handler, param, -1, -1);
}
...
D_Result d_atv_pop_select_show_with_prompt (D_U16 prio,
		...
		D_U32 param) D_FUNCPOSTFIT
{
	return d_atv_pop_select_show_with_prompt_new (prio, layer, str_title, msg, timeout, def, handler, param, -1, -1);
}
