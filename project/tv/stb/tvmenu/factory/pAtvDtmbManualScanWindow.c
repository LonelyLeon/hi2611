...
#include "pMem.h"
#include "pStb.h"
#include "atv_dtmb_manual_scan_window.c"
...
{
    D_Frame *frm = &atv_dtmb_manual_scan_window;
    D_Visual    *vis;
    D_U16 val;
...
/** @brief 事件处理*/
static D_Result
atv_dtmb_manual_scan_window_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    switch (evt->type)
    {
        case D_EVT_FP:
            ...
            switch (evt->fp.key)
            {
                ...
                case D_KEY_BACK:
                    ...
                    break;
                case D_KEY_YELLOW:
                    return p_atv_chan_impulse_show();
                    break;
                default:
                    break;
            }
...
/** @brief 窗口初始化*/
D_Result
atv_dtmb_manual_scan_window_init (D_Frame *frm) D_FUNCPOSTFIT
{
    D_Visual        *vis;
    D_U8    idx;
    D_U8    mode_cnt;
    ...
    if (vis)
    {
        ...
        d_vis_set_handler (vis, atv_dtmb_manual_scan_window_handler);
...
/** @breif 显示信号检测窗口*/
D_Result
p_atv_signal_detect_show (void) D_FUNCPOSTFIT
{
    if(p_conf_get_scan_config())
    {
        return p_atv_dvbc_signal_detect_show ();
    }
    else
    {
        return d_gui_enter_menu (&atv_dtmb_manual_scan_window, D_TRUE);
    }
}
