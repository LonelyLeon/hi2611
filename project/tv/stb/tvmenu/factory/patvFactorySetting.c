...
static D_Result
panel_setting_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    switch (evt->type)
    {
        case D_EVT_ENTER:
...
static D_Result
signal_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    D_Frame *frm = &factory_setting_window;
    D_Visual *vis_label;
    D_VidInput input;
    ...
    input = p_conf_get_vid_input();
    vis_label = d_frame_get_control(frm, ID_FACTORY_LABEL_13);
...
label_vis_show(void) D_FUNCPOSTFIT
{
    ...
    vis = d_frame_get_control(frm, ID_FACTORY_LABEL_13);
    if(vis)
    {
        d_label_set_text(vis, ">>>");
    }
    ...
    if(vis)
    {
        //d_label_set_text(vis,  sw_version);
#ifdef D_TV_IAMGE_SOURCE
        d_label_set_text(vis, "L_HI2611_LD32U3100_CHN");
#else
        d_label_set_text(vis, "L_HI2610_BOE_32B510X_CHN");
#endif
    }
    vis = d_frame_get_control(frm, ID_FACTORY_LABLE_TIME);
    if(vis)
    {
        D_Date_Ext date;
    ...
    vis = d_frame_get_control(frm, ID_FACTORY_LABLE_SVN);
    if(vis)
    {
        memset(svnVersion, 0, sizeof(svnVersion));
        GetSvnVersion(svnVersion);
        d_label_set_text(vis, svnVersion);
    }

    vis = d_frame_get_control(frm, ID_FACTORY_LABEL_BOARD);
    if(vis)
    {
#ifdef D_TV_IAMGE_SOURCE
        d_label_set_text(vis, "HI2611-SM05V06");
#else
        d_label_set_text(vis, "HI2611-SM05V06");
#endif
    }
...
set_vis_handler(void) D_FUNCPOSTFIT
{
    ...
    {
#ifdef D_TV_IAMGE_SOURCE
        d_vis_set_handler(vis, panel_setting_handler);
#else
    ...
    vis = d_frame_get_control(frm, ID_FACTORY_BUTTON_13);
    if(vis)
    {
        d_vis_set_handler(vis, signal_handler);
    }
    return D_OK;
}
