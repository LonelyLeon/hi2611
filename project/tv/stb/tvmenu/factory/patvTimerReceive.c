...
static void aging_ctrl_task(void *p) D_FUNCPOSTFIT
{
    ...
    while(aging_ctrl_running == D_TRUE)
    {
        if(times == 1)
        {
			...
			p_av_set_vid_background(0, D_VID_BackGround_Red, D_TRUE);
        }
        else if(times == 2)
        {
            times = 3;
			...
			p_av_set_vid_background(0, D_VID_BackGround_Green, D_TRUE);
        ...
        else if(times == 3)
        {
			...
			p_av_set_vid_background(0, D_VID_BackGround_Blue, D_TRUE);
...
//            d_lvds_Set_Aging(1, 0xFF, 0xFF, 0xFF);
			p_av_set_vid_background(0, D_VID_BackGround_White, D_TRUE);
        }
        d_task_wait(3000);
    }
}
