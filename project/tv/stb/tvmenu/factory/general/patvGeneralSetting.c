...
typedef struct
{
    ...
    D_U8  logo_mode, pm_mode, standard;
...
set_general_setting_data(D_ID id, D_Bool flag) D_FUNCPOSTFIT
{
    ...
    switch(id)
    {
        ...
        case ID_GENERAL_BUTTON_6:
            if(general_setting_data->standard == 0)
            {
                ...
                general_setting_data->standard = 1;
            }
            else if(general_setting_data->standard == 1)
            {
                vis = d_frame_get_control(frm, ID_GENERAL_LABEL_6);
                if(vis)
                {
                ...
                general_setting_data->standard = 0;
            }
            p_tvconf_set_standard (&general_setting_data->standard);
            d_av_set_atv_test_unstandard(general_setting_data->standard);
            break;
        default:
            break;
    }
...
static D_Result
standard_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    D_Frame *frm = &general_setting_window;
    D_Visual *vis_label;
    D_ID id;
...
get_set_of_general_setting(void) D_FUNCPOSTFIT
{
    ...
    p_tvconf_get_standard(&general_setting_data->standard);
...
general_setting_label_show(void) D_FUNCPOSTFIT
{
    ...
    if(vis)
    {
        d_label_set_text(vis, "NON STANDARD");
    ...
    d_vis_enable(d_frame_get_control(frm, ID_GENERAL_LABEL_SHOW), D_FALSE);
    d_vis_enable(d_frame_get_control(frm, ID_GENERAL_LABEL_1), D_FALSE);
#endif
    ...
    if(vis)
    {
        vis_switch_init(general_setting_data->standard, vis);
    }
...
general_setting_vis_handler(void) D_FUNCPOSTFIT
{
    ...
    if(vis)
    {
        d_vis_set_handler(vis, standard_handler);
    }
