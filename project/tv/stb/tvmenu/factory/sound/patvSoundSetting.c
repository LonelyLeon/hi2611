...
static D_Result
drc_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    switch (evt->type)
    {
        case D_EVT_PRESSED:
...
sound_setting_window_init(D_Frame *frm) D_FUNCPOSTFIT
{
    ...
    if(vis)
    {
        d_button_set_text(vis, "DAC_DEBUG EN");
        d_vis_set_handler(vis, avc_dac_ctl_handler);
    }
    ...
    vis = d_frame_get_control(frm, ID_SOUND_BUTTON_14);
    if(vis)
    {
        d_button_set_text(vis, "DRC");
        d_vis_set_handler(vis, drc_handler);
    }
