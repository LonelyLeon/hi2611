	...
	switch(id)
	{
		case ID_GAMMA_BUTTON_1:
			if(gamma_data->gamma_on== 0)
			{
				...
				gamma_data->gamma_on = 1;
			}
			else if(gamma_data->gamma_on == 1)
			{
				vis = d_frame_get_control(frm, ID_GAMMA_LABEL_1);
				if(vis)
				{
				...
				gamma_data->gamma_on = 0;
			}
			break;
		case ID_GAMMA_BUTTON_2:
			if(flag)
			{
				if(gamma_data->gamma_idx== 7)
					gamma_data->gamma_idx = 0;
				else 
					gamma_data->gamma_idx++;
			}
			else
			{
				if(gamma_data->gamma_idx == 0)
					gamma_data->gamma_idx = 7;
				else 
					gamma_data->gamma_idx--;
			}
			vis = d_frame_get_control(frm, ID_GAMMA_LABEL_2);
			{
				sprintf(gamma_data->gamma_idx_buf, "%d", gamma_data->gamma_idx);
				d_label_set_text(vis, gamma_data->gamma_idx_buf);
			}
			break;
...
static D_Result
gamma_on_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
	D_Frame *frm = &gamma_window;
    D_Visual *vis_label;
	D_ID id;
...
gamma_idx_handler(D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
	...
	id = d_frame_get_control_id(frm,vis);
	vis_label = d_frame_get_control(frm, ID_GAMMA_LABEL_2);
...
gamma_vis_set(void) D_FUNCPOSTFIT
{
	...
	d_label_set_text(vis, "Gamma On");
	d_vis_set_handler(vis, gamma_on_handler);
	
	vis = d_frame_get_control(frm, ID_GAMMA_BUTTON_2);
	d_label_set_text(vis, "Gamma Idx");
	d_vis_set_handler(vis, gamma_idx_handler);
	
	vis = d_frame_get_control(frm, ID_GAMMA_LABEL_1);
	vis_switch_init(gamma_data->gamma_on, vis);
