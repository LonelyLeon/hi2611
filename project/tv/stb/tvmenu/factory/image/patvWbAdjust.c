...
typedef struct
{
    D_S16 temperature, r_gain, g_gain, b_gain, power_ref, r_offset, g_offset, b_offset;
...
{
    wb_adjust_data->tparam.rgain = wb_adjust_data->r_gain;
    wb_adjust_data->tparam.ggain = wb_adjust_data->g_gain;
    wb_adjust_data->tparam.bgain = wb_adjust_data->b_gain;
    wb_adjust_data->tparam.roffset = wb_adjust_data->r_offset;
    wb_adjust_data->tparam.goffset = wb_adjust_data->g_offset;
    wb_adjust_data->tparam.boffset = wb_adjust_data->b_offset;
...
get_set_of_wb_adjust(void) D_FUNCPOSTFIT
{
    ...
    {
        wb_adjust_data->r_gain = wb_adjust_data->tparam.rgain;
        wb_adjust_data->g_gain = wb_adjust_data->tparam.ggain;
        wb_adjust_data->b_gain = wb_adjust_data->tparam.bgain;
        wb_adjust_data->r_offset = wb_adjust_data->tparam.roffset;
        wb_adjust_data->g_offset = wb_adjust_data->tparam.goffset;
        wb_adjust_data->b_offset = wb_adjust_data->tparam.boffset;
    }
    sprintf(wb_adjust_data->r_gain_buf, "%d", wb_adjust_data->r_gain);
    sprintf(wb_adjust_data->g_gain_buf, "%d", wb_adjust_data->g_gain);
    sprintf(wb_adjust_data->b_gain_buf, "%d", wb_adjust_data->b_gain);
    sprintf(wb_adjust_data->r_offset_buf, "%d", wb_adjust_data->r_offset);
    sprintf(wb_adjust_data->g_offset_buf, "%d", wb_adjust_data->g_offset);
    sprintf(wb_adjust_data->b_offset_buf, "%d", wb_adjust_data->b_offset);
...
set_temperature_connected_show(D_S16 flag) D_FUNCPOSTFIT
{
    ...
    d_vip_setTemperature( wb_adjust_data->input,(D_U8*)&wb_adjust_data->tparam, (D_U8*)&wb_adjust_data->cscparam);
		...
		d_label_set_text(vis,wb_adjust_data->r_gain_buf);

		vis = d_frame_get_control(frm, ID_WB_ADJUST_LABEL_5);
		d_label_set_text(vis,wb_adjust_data->g_gain_buf);

		vis = d_frame_get_control(frm, ID_WB_ADJUST_LABEL_6);
		d_label_set_text(vis,wb_adjust_data->b_gain_buf);

		vis = d_frame_get_control(frm, ID_WB_ADJUST_LABEL_8);
		d_label_set_text(vis,wb_adjust_data->r_offset_buf);

		vis = d_frame_get_control(frm, ID_WB_ADJUST_LABEL_9);
		d_label_set_text(vis,wb_adjust_data->g_offset_buf);

		vis = d_frame_get_control(frm, ID_WB_ADJUST_LABEL_10);
		d_label_set_text(vis,wb_adjust_data->b_offset_buf);
    return D_OK;
}
...
set_wb_adjust_data(D_ID id, D_Bool flag) D_FUNCPOSTFIT
{
    ...
    switch(id)
    {
        case ID_WB_ADJUST_BUTTON_1:
            ...
            if(flag)
            {
                if(wb_adjust_data->temperature == 2)
                    wb_adjust_data->temperature = 0;
                else
                    wb_adjust_data->temperature++;
            }
            else
            {
                if(wb_adjust_data->temperature == 0)
                    wb_adjust_data->temperature = 2;
                else
                    wb_adjust_data->temperature--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_4:
            if(flag)
            {
                if(wb_adjust_data->r_gain == 255)
                    wb_adjust_data->r_gain = 0;
                else
                    wb_adjust_data->r_gain++;
            }
            else
            {
                if(wb_adjust_data->r_gain == 0)
                    wb_adjust_data->r_gain = 255;
                else
                    wb_adjust_data->r_gain--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_5:
            if(flag)
            {
                if(wb_adjust_data->g_gain == 255)
                    wb_adjust_data->g_gain = 0;
                else
                    wb_adjust_data->g_gain++;
            }
            else
            {
                if(wb_adjust_data->g_gain == 0)
                    wb_adjust_data->g_gain = 255;
                else
                    wb_adjust_data->g_gain--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_6:
            if(flag)
            {
                if(wb_adjust_data->b_gain == 255)
                    wb_adjust_data->b_gain = 0;
                else
                    wb_adjust_data->b_gain++;
            }
            else
            {
                if(wb_adjust_data->b_gain == 0)
                    wb_adjust_data->b_gain = 255;
                else
                    wb_adjust_data->b_gain--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_8:
            if(flag)
            {
                if(wb_adjust_data->r_offset == 255)
                    wb_adjust_data->r_offset = 0;
                else
                    wb_adjust_data->r_offset++;
            }
            else
            {
                if(wb_adjust_data->r_offset == 0)
                    wb_adjust_data->r_offset = 255;
                else
                    wb_adjust_data->r_offset--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_9:
            if(flag)
            {
                if(wb_adjust_data->g_offset == 255)
                    wb_adjust_data->g_offset = 0;
                else
                    wb_adjust_data->g_offset++;
            }
            else
            {
                if(wb_adjust_data->g_offset == 0)
                    wb_adjust_data->g_offset = 255;
                else
                    wb_adjust_data->g_offset--;
            }
        ...
        case ID_WB_ADJUST_BUTTON_10:
            if(flag)
            {
                if(wb_adjust_data->b_offset == 255)
                    wb_adjust_data->b_offset = 0;
                else
                    wb_adjust_data->b_offset++;
            }
            else
            {
                if(wb_adjust_data->b_offset == 0)
                    wb_adjust_data->b_offset = 255;
                else
                    wb_adjust_data->b_offset--;
            }
