...
typedef struct
{
    ...
    D_Char  mediaPvrFolderShowName[MEDIA_PVR_LIST_ROW_CNT][MEDIA_PVR_FILE_NAME_LEN];
...
static D_Result
media_pvr_window_show(D_Bool show)D_FUNCPOSTFIT
{
    d_vis_show(d_frame_get_control(&atv_media_pvr_window, ID_PVR_GROUP_ALL), show);
    return D_OK;
}
...
#endif
            p_atv_media_device_name_map(pvr_data->mediaPvrListShowData[row]);
...
media_pvr_fullscreen_hide_handler(D_U16 focus)D_FUNCPOSTFIT
{
    if(pvr_data)
    {
        media_pvr_window_show(D_TRUE); // 退出全屏时，要恢复列表窗口的显示 @qiny 2017.11.16
        media_pvr_stop_play();
                ...
                case D_KEY_PNL_OK:
                    ...
                    else if(Judge_PVR_File(&(pvr_data->mediaPvrFileRset.ids[pvr_data->mediaPvrCurrListFocus])))
                    {
                        ...
                        if(D_OK == p_media_pvr_full_screen_show(pvr_data->mediaPvrFileRset.ids[pvr_data->mediaPvrCurrListFocus].path.filename, &pvr_data->mediaPvrFileRset , pvr_data->mediaPvrCurrListFocus, media_pvr_fullscreen_hide_handler))
                        {
                            ...
                            media_pvr_window_show(D_FALSE); // 全屏时，要隐藏列表窗口的显示 @qiny 2017.11.16
                        }
                    }
                    return D_OK;
                case D_KEY_DOWN:
