...
#define MEDIA_EBOOK_LIST_ROW_CNT  (10)
...
typedef struct
{
    ...
    D_Visual* vis_ebook_list, *vis_ebook_logo, *vis_ebook_path_name;
} MediaEBookData;
...
media_ebook_get_img_id_by_filetype(D_FSAttr *fattr) D_FUNCPOSTFIT
{
    ...
    switch(fattr->class_type)
    {
        ...
        case D_FILE_SCAN_FMT_EBK:
            img_id = atv_IMG_ID_ebk_icon_focus_small;
            break;
        case D_FILE_SCAN_FMT_Txt:
            img_id = atv_IMG_ID_txt_icon_focus_small;
            break;
...
#endif
            p_atv_media_device_name_map(media_ebook_data->mediaEBookListShowData[row]);
...
media_ebook_window_window_handler (D_Visual *vis, D_Event *evt) D_FUNCPOSTFIT
{
    ...
    switch(evt->type)
    {
        case D_EVT_USER:
            ...
            {
                media_ebook_data->mediaEBookCurrLang = evt->user.param2;
                if(D_LANG_CHN == evt->user.param2)
                {
                    d_image_box_set_image(media_ebook_data->vis_ebook_logo, d_gui_get_image(atv_IMG_ID_ebook_title_cn));
                }
                else
                {
                    d_image_box_set_image(media_ebook_data->vis_ebook_logo, d_gui_get_image(atv_IMG_ID_ebook_title_en));
                }
                d_vis_update(media_ebook_data->vis_ebook_logo, NULL);
                d_gui_update();
            }
            break;
        case D_EVT_FP:
...
atv_media_ebook_window_init(D_Frame *frm) D_FUNCPOSTFIT
{
    ...
    media_ebook_data->vis_ebook_logo = d_frame_get_control(frm, ID_EBOOK_IMG_LOGO);
    ...
    if(1 == media_ebook_data->mediaEBookCurrLang)
    {
        d_image_box_set_image(media_ebook_data->vis_ebook_logo, d_gui_get_image(atv_IMG_ID_ebook_title_cn));
    }
    else
...
{
    return d_gui_enter_menu_new(ATV_MEDIA_MENU_ID, &atv_media_ebook_window, D_CLEAR_ALL);
}
