...
static D_Result atv_media_pvr_record_stop(void) D_FUNCPOSTFIT;
...
#if 0
    if(info->pcr_pid != 0x1FFF && info->pcr_pid != info->vid_pid && info->pcr_pid != info->aud1_pid)
    {
        pvrconfig->pvr_rec_pid[5] = info->pcr_pid;
    }
    pvrconfig->pvr_rec_pid[6] = PID_PAT;            // 录制PAT
    pvrconfig->pvr_rec_pid[7] = info->pmt_pid;   // 录制对应的PMT
#endif
