...
typedef struct
{
    ...
    D_Result (*handler) (D_Char *file_path, D_ID cur_index) D_FUNCPOSTFIT;
...
static D_FileScan           *picture_fs = NULL;
...
search_picture_next_or_prev(D_Bool flag)
{
    ...
    for(i = 0; i < picture_fs->cnt; i++)
    {
        if (flag)
        {
            if (picture_full_data->pic_index < (picture_fs->cnt - 1))
                picture_full_data->pic_index++;
            else
                picture_full_data->pic_index = 0;
            if ( p_file_scan_get_file_class_type(1, picture_fs->ids[picture_full_data->pic_index].path.filename) == D_FILE_SCAN_FMT_Photo)
                break;
        }
        else
        {
            if (picture_full_data->pic_index >= 1)
                ...
                picture_full_data->pic_index = picture_fs->cnt - 1;
            if ( p_file_scan_get_file_class_type(1, picture_fs->ids[picture_full_data->pic_index].path.filename) == D_FILE_SCAN_FMT_Photo)
                break;
        }
    }
    ...
    ptr = picture_full_data->next_pic_file_photo_name;
    strcpy(picture_full_data->next_pic_file_photo_name, picture_full_data->file_path);
    ptr += strlen(picture_full_data->file_path);
    strcpy(ptr, picture_fs->ids[picture_full_data->pic_index].path.filename);
...
void p_set_picture_rotate(D_U8 flag)
{
    ...
    if(D_OK == d_media_picture_get_info(picture_full_data->next_pic_file_photo_name, &jpg_info))
    {
        ...
        if(fsattr->file_type == D_FILE_TYPE_JPG)
        {
            d_media_picture_stop();

            p_gui_auto_match_window_format(&(Win.x), &(Win.y), &(Win.w), &(Win.h));            
            if( D_OK == d_media_picture_play(picture_full_data->next_pic_file_photo_name, Win.x,Win.y, Win.w, Win.h, 0, flag, PICTURE_OPEN_MODE_NORMAL))
            {
				d_av_set_vid_blank(0, D_FALSE); /*退出黑屏*/
            }
        }
        else if(fsattr->file_type != D_FILE_TYPE_GIF && fsattr->file_type != D_FILE_TYPE_JPG)
        {
            // if(fsattr->file_type == D_FILE_TYPE_PNG || fsattr->file_type == D_FILE_TYPE_BMP)
            // {
            //     d_image_box_set_image(picture_full_data->vis_loading, d_gui_get_image(atv_IMG_ID_loading));
            // }
            //  p_gui_auto_match_window_format(&(Win.x), &(Win.y), &(Win.w), &(Win.h));
...
void p_browse_picture(D_Bool next_or_prev) //false 为上一张图片，true为下一张
{
    ...
    if(next_or_prev)
    {
        ...
        if(D_OK == d_media_picture_get_info(picture_full_data->next_pic_file_photo_name, &picture_full_data->picture_info))
        {
            ...
            if(D_OK == d_media_picture_play(picture_full_data->next_pic_file_photo_name, Win.x,
					Win.y, Win.w, Win.h, 0, 0, PICTURE_OPEN_MODE_NORMAL))
			{
				d_av_set_vid_blank(0, D_FALSE); /*退出黑屏*/
			}
        ...
        case D_EVT_FP:
        {
            switch (evt->fp.key)
            {
                ...
                case D_KEY_ESC:
                {
                    ...
                    {
                        picture_full_data->handler(picture_full_data->next_pic_file_photo_name, picture_full_data->pic_index);
                    }
...
/** @brief 全屏播放图片控制窗口显示*/
D_Result p_pic_full_screen_menu_show(D_Char *path , D_FileScan *fs, D_ID index, D_Result (*handler) (D_Char *out_path)D_FUNCPOSTFIT)D_FUNCPOSTFIT
{
    D_Char *tok;
    D_Result ret = D_ERR;
