...
typedef struct
{
    ...
    D_U32 last_play_time;
    D_Char prompt_last_time[128];
...
static D_Result media_full_movie_reset_show_timer(void) D_FUNCPOSTFIT;
...
extern D_Result 
p_atv_media_last_movie_save(D_Char* pathname, D_U32 play_time);
extern D_U32
p_atv_media_last_movie_check(D_Char* pathname);
...
media_full_mvoie_set_play_status(D_U8 status)D_FUNCPOSTFIT
{
    ...
    switch(status)
    {
        case MEDIA_FULL_MOVIE_START_STATUS:
            ...
            d_image_box_set_image(media_full_movie_data->vis_play_status, d_gui_get_image(atv_IMG_ID_media_pause));
        ...
        case MEDIA_FULL_MOVIE_PAUSE_STATUS:
            ...
            d_media_movie_pause();
            d_image_box_set_image(media_full_movie_data->vis_play_status, d_gui_get_image(atv_IMG_ID_media_play));
...
media_full_movie_update_show_time(D_Bool show, D_Bool reset) D_FUNCPOSTFIT
{
    ...
    if(media_full_movie_data->mediaFullMovieCurrTimeValue > 1.0 && media_full_movie_data->mediaFullMovieCurrTimeValue < 360000.0)
    {
        cur_sec = (D_U32)media_full_movie_data->mediaFullMovieCurrTimeValue;
        p_atv_media_last_movie_save(NULL, cur_sec);
    }
...
static D_Result
media_seek_last(D_Bool sel)D_FUNCPOSTFIT
{
    if(sel && media_full_movie_data && media_full_movie_data->last_play_time > 10 && media_full_movie_data->last_play_time > (D_U32)media_full_movie_data->mediaFullMovieCurrTimeValue)
    {
        d_media_movie_seek(media_full_movie_data->last_play_time - 1);
        if(media_full_movie_data->mediaFullMovieMenuEnable == MEDIA_FULL_MOVIE_MENU_HIDE)
        {
...
media_full_movie_play(D_Bool first)D_FUNCPOSTFIT
{
    ...
    media_full_movie_data->last_play_time = 0;
    media_full_movie_data->last_play_time = p_atv_media_last_movie_check(media_full_movie_data->mediaFullMoviePathName);
    p_atv_media_last_movie_save("", 0);
...
_MOVIE_PLAY_END:
    ...
    if(ret == D_OK)
    {
        p_atv_media_last_movie_save(media_full_movie_data->mediaFullMoviePathName, 0);
        if(media_full_movie_data->last_play_time > 10 && media_full_movie_data->last_play_time < (D_U32)(media_full_movie_data->mediaFullMovieInfo.totaltime - 2))
        {
            D_U32 cur_hour, cur_min, cur_sec;
            D_Char strtime[16];
            cur_hour  = (D_U32)media_full_movie_data->last_play_time / 3600;
            cur_min  = ((D_U32)media_full_movie_data->last_play_time % 3600) / 60;
            cur_sec  = ((D_U32)media_full_movie_data->last_play_time % 3600 ) % 60;
            snprintf(strtime, sizeof(strtime), "%02d:%02d:%02d", cur_hour, cur_min, cur_sec);
            snprintf(media_full_movie_data->prompt_last_time, sizeof(media_full_movie_data->prompt_last_time), d_gui_get_text(atv_IDS_GOTO_LAST_TIME), strtime);
            d_atv_pop_select_show_with_prompt_new (D_POP_MSG_PRIO_MSG_3, D_GUI_LAYER_MESSAGE, 
                                      d_gui_get_text(atv_IDS_PROMPT_INFO), media_full_movie_data->prompt_last_time, 
                                      20000, D_FALSE, media_seek_last, 0, 438, 250);
        }
    }
...
media_full_movie_play_by_mode(D_U8 flag)D_FUNCPOSTFIT
{
    ...
    d_atv_pop_select_hide(D_POP_MSG_PRIO_MSG_3);
    media_full_movie_update_show_time(D_FALSE, D_TRUE);
...
atv_media_movie_full_window_init (D_Frame *frm) D_FUNCPOSTFIT
{
    ...
    if(vis)
    {
        ...
        media_full_movie_data->mediaFullMovieWin.w = media_full_movie_data->vis_full_win->rect.w;
        media_full_movie_data->mediaFullMovieWin.h = media_full_movie_data->vis_full_win->rect.h;
