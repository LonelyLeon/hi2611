...
typedef enum {
  DLNA_PS_NO_MEDIA_PRESENT,
  DLNA_PS_PLAYING,
  DLNA_PS_PAUSED_PLAYBACK,
  DLNA_PS_STOPPED,
  DLNA_PS_TRANSITIONING
} dlna_play_status_code_t;
...
/**
 * Output the protocol information string that must be send by a DMR to a DMC
...
void *dlna_dmr_stateloop(void *arg);
/***************************************************************************/
/*                                                                         */
 ...
 * @param[in] dlna  The DLNA library's controller.
 */
void dlna_device_set_uuid (dlna_t *dlna);
