...
/*以下类型声明和德赛CA重复，重新放置到hrca_config.h中*/
/*
...
返 回 值: 未读邮件的个数                     
*/
INT32 DIVI_GetNewMailCount(void);
...
返 回 值： 随机数 
*/
extern UINT32 DIVI_GetRandomNumber(void);
...
其他说明:无
*/
INT32 DIVI_Active_ChildCard(void);
/*提供者：CA模块
使用者：机顶盒
何时调用：停止进行子母卡操作时
功能描述：停止进行子母卡操作
...
其他说明:无
*/
INT32 DIVI_Finish_ChildCard(void);
...
//2017-12-18, 用于控制OTT或清流节目有无收视权限
//返回值为1: 表示DVB基础产品包有授权
//返回值为0: 表示DVB基础产品包无授权或卡未插入
//对于OTT建议每次退出DVB时获取该函数返回值并保存
//如果为清流节目，则根据该函数返回值判断是否具备收看权限
BOOL DIVI_GetProductOneIs();

//清除机卡配对，仅限调试使用
INT32 DIVI_ClearPaired();

//V4: 根据海尔要求增加OSDtask的Sleep函数，2016-12-19
extern void DIVISTBCA_Sleep(UINT32 wMilliSeconds);

//以下两个函数暂置为空
extern BOOL DIVI_Show_Finger(UINT8 Position,UINT8 Duriaon,UINT8 * Finger_Content,UINT8 Content_Length);

extern INT8 DIVI_GetFingerNum(void);

//20170227 nocard
extern INT32 DIVI_PLUS_Initialize(void);

extern INT32  DIVI_PLUS_ResetCard(UINT8 bCardNumber,UINT8 * ATRBuf,UINT8 BufferLen,UINT8 * RLength);

extern UINT8 DIVI_PLUS_Get_CardState(void);

extern INT32 DIVI_PLUS_SendDataEx(UINT8 bCardNumber,UINT8 bLength,
	                                                      UINT8 * pabMessage,
	                                                      UINT8 * pabResponse,
	                                                      UINT8 RBufferLen,
	                                                      UINT8 * bRLength,
...
返 回 值： 
           ...
           0x00: 获取失败
*/
extern INT32 DIVI_GetChipID(UINT8 * IcID, UINT8 Len);

/*-------------------------------------------------------------------
函数功能: DES解密，使用OTP中的PDK解密数据得到KxCW1
输入参数: SrcData为原始数据(长度为8的整数倍)，DsData为目标数据，SrcLen原始数据长度
输出参数: 返回结果
返回值:   1:解密成功,0:解密失败
--------------------------------------------------------------------*/
extern int DIVI_SetKxCW1(unsigned char *ScrData,unsigned char *DsData,int SrcLen);

/*-------------------------------------------------------------------
函数功能: DES解密，使用OTP中的PDK解密数据得到KxCW2
输入参数: SrcData为原始数据(长度为8的整数倍)，DsData为目标数据，SrcLen原始数据长度
输出参数: 返回结果
返回值:   1:解密成功,0:解密失败
--------------------------------------------------------------------*/
extern int DIVI_SetKxCW2(unsigned char *ScrData,unsigned char *DsData,int SrcLen);

/*-------------------------------------------------------------------
函数功能: DES解密，使用KxCW1或KxCW2解密数据得到CW
输入参数: 
                  Key_id为编号，若为1则使用KxCW1，若为2则使用KxCW2
                  SrcData为原始数据(长度为8的整数倍)，DsData为目标数据，SrcLen原始数据长度
输出参数: 返回结果
返回值:   1:解密成功,0:解密失败
--------------------------------------------------------------------*/
extern int DIVI_SetKyCW1(UINT16 Key_id,unsigned char *ScrData,unsigned char *DsData,int SrcLen);
...
返回值:   1:解密成功,0:解密失败
--------------------------------------------------------------------*/
extern int DIVI_SetKyCW2(UINT16 Key_id,unsigned char *ScrData,unsigned char *DsData,int SrcLen);
