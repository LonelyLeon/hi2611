...
//#define //#define SUPPORT_CARD_STB_PAIR  有去掉这个功能，否则机顶盒会有很多卡提示机卡不配对
...
#define   CA_TASK_PRIORITY1   9// 低于gui task 以免 ECM parse 线程抢占CA线程
#define   SC_TASK_PRIORITY1   7// 要高于CA_TASK_PRIORITY，因为可能会有 card resum 调用
...
#define     SC_EXIRATION_REMINDER  17 /*到期提醒!*/
...
/* -------------------------------------service info-------------------------------*/
#define XSMCA_MAX_SERVICE_PER_ECM 1
typedef struct CAServiceInfo {
	UINT16 m_wEcmPid;
	UINT8 m_bServiceCount;//默认填1
	UINT16 m_wServiceId[XSMCA_MAX_SERVICE_PER_ECM];
}XinShiMao_CAServiceInfo;

//------------------------------------------------------------------------------------
typedef struct CAOSDInfo{
...
UINT32 __GmtTimeToMJD(INT32 Y,INT32 MO,INT32 D);

UINT8 XinShiMao_Mjd2Gmt(UINT16 Mjd ,UINT8 BcdH,UINT8 BcdM,UINT8 BcdS,TXinShiMao_DATETIME* ptGmt);
