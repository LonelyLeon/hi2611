...
#define		MESSAGE_NEED_PAIR_TYPE					0x15		/*需要机卡配对*/
#define		MESSAGE_TEST_CARD_TYPE					0x16		/*您的卡处于测试状态*/
...
#define		MESSAGE_SYSTEM_DATA_ERR_TYPE			0x38		/*系统异常，无法更新*/
#else
#define		MESSAGE_STB_NEED_UPDATE_TYPE		    0x38		/*机顶盒需要升级，请稍等*/
#endif
#define		MESSAGE_CARD_IN_BLACKLIST_TYPE			0x39		/*您的卡已被列入黑名单！*/

#if defined(DSCA_56_OVERSEAS)
#define		MESSAGE_STB_NEED_UPDATE_TYPE			0x3A		/*机顶盒需要升级，请稍等*/
#endif
...
#define		MESSAGE_SHOW_FORCEOSD_TYPE				0x63		/*显示强制OSD消息*/
#define		MESSAGE_HIDE_FORCEOSD_TYPE				0x64		/*隐藏强制OSD消息*/
...
#define		DSCA_IPPV_NAME_LEN					(32)
#define		DSCA_MAIL_TITAL_LEN					(32)
...
#endif
/*以下变量声明和数视通CA重复，放置到hrca_config.h中定义*/
/*
typedef char			INT8;
...
#if (defined(DSCA_56_OVERSEAS))
	UINT8				aucMailTitle[DSCA_MAIL_TITAL_LEN*3 + 1];		// 邮件标题，最长为30
	UINT8				aucMailSender[DSCA_MAIL_TITAL_LEN*3 + 1];	//邮件发送人
#else
...
{
#if (defined(DSCA_56_OVERSEAS))
	UINT8 aucOperatorName[DSCA_OPERATOR_NAME_LEN*3];
	UINT8 aucOperatorPhone[DSCA_OPERATOR_PHONE_LEN*3];
	UINT8 aucOperatorAddress[DSCA_OPERATOR_ADDRESS_LEN*3]; 
#else
...
{
#if (defined(DSCA_56_OVERSEAS))
	UINT8			aucIppvName[DSCA_IPPV_NAME_LEN*3+1];
#else
...
//字体大小
typedef enum _EN_SHOW_FONT_TYPE
{
	SHOW_FONT_8 = 1,
	SHOW_FONT_12,
	SHOW_FONT_16,
...
#if (defined(DSCA_56) || defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
	EN_SHOW_FONT_TYPE enShowFontSize;					/*显示字体大小*/
#endif
...
#if (defined(DSCA_56) || defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
	UINT8 ucShowAttribute;					/*显示属性*/
	UINT8 ucColorType;						/*颜色类型*/
	UINT8 ucShowBCFlg;						/*是否显示背景色*/
#else
...
typedef enum _EN_OSD_SHOW_POS
{
	...
	SHOW_BY_RATIO,				//全屏OSD,按照一定屏幕比例显示
}EN_OSD_SHOW_POS;
...
#if (defined(DSCA_56) || defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
	EN_SHOW_FONT_TYPE 	enFontSize;
	UINT8				ucTransparency;//透明度
	UINT8				ucColorType;
	UINT8				ucScreenRatio;
#endif
	UINT8				ucFontType;
...
#if (defined(DSCA_52) || defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
...
#if (defined(DSCA_56) || defined(DSCA_COS)|| defined(DSCA_56_OVERSEAS))
	CA_ERROR_CARD_INBLACKLIST,		/*智能卡处于黑名单中*/
#endif
...
#if defined(DSCA_COS)//暂时注释掉，需要整理到菜单
	CA_ERROR_CARD_TIMEOUT,			/*智能卡超时*/
#endif
#if defined(DSCA_52)
	CA_ERROR_STB_NEED_UPDATE,		/*机顶盒需要升级*/
#endif
#if defined(DSCA_56_OVERSEAS)
	CA_ERROR_NOT_SUPPORT,			/*不支持此功能*/
    CA_ERROR_STB_NEED_UPDATE,		/*机顶盒需要升级*/
#endif
...
extern BOOL DSCA_GetBuffer(OUT UINT32 * puiStartAddr,  OUT UINT32 * puiSize);
extern BOOL DSCA_FlashRead(UINT32 uiStartAddr,  OUT UINT8 * pucData,  IN UINT32 * puiLen);
extern BOOL DSCA_FlashWrite(UINT32 uiStartAddr, IN UINT8* pucData, UINT32 uiLen);
extern BOOL DSCA_FlashErase(void);
extern void* DSCA_Malloc(UINT32 uiBufferSize);
extern BOOL DSCA_Free(IN void* pucBuffer);
extern BOOL DSCA_Memset(IN void* pucBuffer, UINT8 ucValue, UINT32 uiSize);
extern BOOL DSCA_Memcpy(IN void* pucDestBuffer, IN void* pucSourceBuffer, UINT32 uiSize);
...
extern BOOL DSCA_Sleep(UINT32 uiMilliSeconds);				       
extern BOOL DSCA_SemaphoreInit(IN DSCA_Semaphore* puiSemaphore , UINT8 ucInitVal);
extern BOOL DSCA_ReleaseSemaphore(IN DSCA_Semaphore* puiSemaphore);
extern BOOL DSCA_WaitSemaphore(IN DSCA_Semaphore* puiSemaphore);
extern BOOL DSCA_ShowMessage(UINT8 ucMessageType, EN_CAS_SHOW_MSG_TYPE  enShowType, IN UINT8 * pucMessage);
#if (defined(DSCA_56))
extern BOOL DSCA_SetCW(UINT16 usEcmPID, IN UINT8* pucOddKey, IN UINT8* pucEvenKey, IN UINT8* pucKeyLen, UINT8 ucBySKOrSN);
#elif (defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
extern BOOL DSCA_SetCW(UINT16 usEcmPID, IN UINT8* pucOddKey, IN UINT8* pucEvenKey, IN UINT8* pucKeyLen);
#endif
extern BOOL DSCA_StopCurrentProgram(void);
extern BOOL DSCA_PlayCurrentProgram(void);
extern BOOL DSCA_SendDataToSmartCard(IN UINT8* pucSend, IN UINT8 *pucSendLen, OUT UINT8* pucReceive,
											OUT UINT8 *pucReceiveLen, UINT32 uiTimeout, OUT UINT8* pucStatusWord );
extern BOOL DSCA_SMFunction(IN UINT8* pucSend, IN UINT8 *pucSendLen, OUT UINT8* pucReceive,
								OUT UINT8 *pucReceiveLen, UINT32 uiTimeout);
...
extern BOOL DSCA_MsgQueueInit(IN INT8* pucName, IN UINT32* uiMsgQueueHandle, UINT32 uiMaxMsgLen, EN_CA_MSG_QUEUE_MODE enMode);
extern BOOL DSCA_MsgQueueGetMsg(UINT32 uiMsgHandle, IN ST_CA_MSG_QUEUE* pstMsg, EN_CA_MSG_QUEUE_MODE enMode, UINT32 uiWaitMilliSecond);
extern BOOL DSCA_MsgQueueSendMsg(UINT32 uiMsgHandle, IN ST_CA_MSG_QUEUE* pstMsg);
extern BOOL DSCA_GetSTBID(OUT UINT8 aucStbId[16], OUT UINT8 *pucIdLen);
#else
extern void DSCA_printf(IN const INT8 *string,...);
...
#if (defined(DSCA_56_OVERSEAS))
EN_ERR_CAS DSCA_Init(UINT8 ucPriority, EN_CA_LANGUAGE enLanguage, EN_CA_FONT_TYPE enFontCodeType);
#else
...
#if (defined(DSCA_COS) || defined(DSCA_56_OVERSEAS))
EN_ERR_CAS DSCA_GetExternalCardNo(UINT8 pucCardNo[36], UINT8* pucCardType);
EN_ERR_CAS DSCA_GetExternalMasterCard(UINT8 pucMasterCardNO[36]);
#endif
EN_ERR_CAS DSCA_GetMasterCard(UINT8 * pucMasterCardNO);
EN_ERR_CAS DSCA_GetCAVer(OUT UINT8 * pucCAVersion);
EN_ERR_CAS DSCA_GetCASystemId(UINT8 aucCASystemId[12]);
...
#if (defined(DSCA_56_OVERSEAS))
EN_ERR_CAS DSCA_GetEmailContent(UINT32 uiEmailID, OUT UINT8* pucContentBuf, UINT16 usBufferLen, OUT UINT16 *pusContentLen);
#else
