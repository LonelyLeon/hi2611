...
/*++++
宏使用说明:	用DVTCA_SECURITYCHIP宏来区分是不是开通了安全功能的安全芯片.由数码视讯人员控制
开启宏:		开启高安功能
关闭宏:		关闭高安功能
----*/
#if (defined(DVTCA_SECURITY))
#define DVTCA_SECURITYCHIP
#else
//#define DVTCA_SECURITYCHIP
#endif
...
#define DVTCA_LEN_SMARTCARD_SN					64		//最长智能卡序列号长度（外部卡号）
...
#if defined(DVTCA_COS_10)
#define DVTCA_STB_EMM_CHIPID_SLOT				5  		//开户命令
#endif
//智能卡状态定义
...
//+++++++++++++++++++++++++++++++++HSCAS新增
//HSCAS机顶盒返回值基值定义
#define DVTHSCAERR_BASE_STB						0xF100F100

#define DVTHSCAERR_TERMINAL_NOT_ACTIVATE		(DVTHSCAERR_BASE_STB + 1)	//终端未激活
#define DVTHSCAERR_TERMINAL_NOT_SERVER			(DVTHSCAERR_BASE_STB + 2)	//终端被禁用
#define DVTHSCAERR_FREEPREVIEW_OVER				(DVTHSCAERR_BASE_STB + 3)	//本次免费预览已结束
#define DVTHSCAERR_FREEPREVIEW_ALLOVER			(DVTHSCAERR_BASE_STB + 4)	//预览总次数已到
#define DVTHSCAERR_AREA_RESTRICT					(DVTHSCAERR_BASE_STB + 5)	//运营商限制区域观看
#define DVTHSCAERR_NO_ENTITLE						(DVTHSCAERR_BASE_STB + 6)	//没有购买此节目Program not paid
#endif
...
//zp:20150319
#define DVTCA_PAIR_SUCCESS							32			//配对成功
#define DVTCA_PAIR_FAILED							33			//配对失败
//wy:20071022－用户自定义数据类型
#define DVTCA_BOUQUETID							254			//自定义数据类型-Bouquet_id

#if (defined(DVTCA_COS_10) || defined(DVTCA_COS_20))
//+++++++++++++++++++++++++++++++++HSCAS新增
#define DVTHSCA_TERMINAL_NOT_ACTIVATE			101			//终端未激活terminal is not activated
#define DVTHSCA_TERMINAL_NOT_SERVER				102			//终端被禁用terminal is forbidden 
#define DVTHSCA_FREEPREVIEW_OVER					103			//本次预览结束preview is over
#define DVTHSCA_FREEPREVIEW_ALLOVER				104			//本节目今日预览次数已到today's preview of this program has been used up  
//----------------------------------------------
#endif
...
#if (defined(DVTCA_BASE) || defined(DVTCA_SECURITY) || defined(DVTCA_COS_10))
...
typedef struct _SDVTCAEmailInfo{
	...
	char	m_szTitle[DVTCA_MAXLEN_EMAIL_SHORT_CONTENT];		//邮件短内容，厂家需要对该内容的编码格式进行校验
}SDVTCAEmailInfo;
