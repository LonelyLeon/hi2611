...
typedef enum
{
	...
	MG_ERR_ZONE_CODE,
	MG_ERR_AUTHNEARBOOT_REMIND      = -57,		/* Added at v4.2.3 */
	MG_ERR_AUTHNEARTIME_REMIND		= -56,       /* Added at v4.2.3 */
	MG_ERR_AUTHEXPIRE_REMIND		= -55,       /* Added at v4.2.0 */
...
extern MG_U8	MG_Get_PinErr_Status(MG_U8 *MG_TimesLeft); //no use
#endif
extern MG_U8 	MG_Get_Program_CostPerMin(MG_U8 MG_PVName);
extern MG_U16 	MG_Get_FirstUseDay(void);//no use
/* <=== */

extern void		MG_Get_STB_Serial(MG_U8 *MG_STBSerials);
extern MG_U8	MG_Get_Card_Ready(void);
extern MG_U8	MG_Get_Lock_Status(void);//no use
extern MG_U8	MG_Get_Partner_Status(void);
extern void		MG_Get_Card_SN(MG_U8 *MG_CardSN);
extern MG_U8	MG_Get_Card_Version(void);//no use
extern void		MG_Get_Group_ID(MG_U8 *MG_GID);//no use
extern void		MG_Get_MSCard_Info(MG_U8 *MG_MSCardInfo);//no use
extern MG_U16	MG_Get_Card_Key_Version(void);//no use
...
extern void 	MG_Set_CurTeleviewPPVTag(MG_U8 MG_State);//no use
extern MG_U8 	MG_Get_TeleviwPPVTag(MG_U8 MG_PVName);
extern void 	MG_Clean_LastPNo_Info(void);//no use
...
extern MG_U8 	MG_Get_Program_Auth(MG_U8 MG_PVName);//no use
extern MG_U8* 	MG_Get_LoaderInfo(void);//no use
extern MG_U16	MG_Get_CASoft_Version(void);
extern void		MG_Get_ServiceID(MG_U8 *MG_ServiceID);//no use
